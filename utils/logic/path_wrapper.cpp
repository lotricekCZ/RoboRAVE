...
std::pair<uint8_t, std::array<motors::motor, 2>> path_wrapper::translate(){
	...
	switch(this -> at(head)._type){
		case step::circle_e: {
			...
			ret.at(0).high_interval = ret.at(0).low_interval = std::floor(std::abs(now_speeds.to_hw_speed(s.left)));
			ret.at(1).high_interval = ret.at(1).low_interval = std::floor(std::abs(now_speeds.to_hw_speed(s.right)));
